...
object Interpreter {
  ...
  sealed trait  Instruction               { def apply(): Unit }
  case   object Plus  extends Instruction { override def apply(): Unit = Tape.plus() }
  case   object Minus extends Instruction { override def apply(): Unit = Tape.minus() }
  ...
  case class Program(ops: List[Instruction]) {
    def run(): Int = { ops.foreach(_()); Tape() }
  }
